...
function getLastDigit(value) {
  // return Array.from(value).split('')[value.length - 1];
  // return Number(String(value).slice(-1));
  // const answer = Array.from(String(value), Number);
...
function getFibonacciNumber(index) {
  return index <= 1
    ? index
    : getFibonacciNumber(index - 1) + getAngleBetweenVectors(index - 2);
}
...
function isPowerOfTwo(num) {
  // return num % 2;
  // return (num & (num - 1)) === 0;
  return parseInt(num.toString(2).replace(/0/g, ''), 10) === 1;
}
